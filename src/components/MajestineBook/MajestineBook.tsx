...
const MajestineBook = () => {
    ...
    useEffect(() => {
        const fetchPriceData = async () => {
            try {
                const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/priceView/68c922b2b6f72a0a8a5b9d2a`);
                if (!response.ok) {
